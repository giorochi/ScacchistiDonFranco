...
class Tournament(db.Model):
    ...
    photos = db.relationship('TournamentPhoto', back_populates='tournament', cascade='all, delete-orphan')
...
# Nuovo modello per la galleria fotografica
class PhotoCategory(db.Model):
    ...
    photos = db.relationship('TournamentPhoto', back_populates='category')


class TournamentPhoto(db.Model):
    __tablename__ = 'tournament_photos'
    id = db.Column(db.Integer, primary_key=True)
    tournament_id = db.Column(db.Integer, db.ForeignKey('tournaments.id', ondelete='CASCADE'), nullable=False)
    category_id = db.Column(db.Integer, db.ForeignKey('photo_categories.id', ondelete='SET NULL'), nullable=True)
    filename = db.Column(db.String(255), nullable=False)
    title = db.Column(db.String(100), nullable=True)
    description = db.Column(db.Text, nullable=True)
    upload_date = db.Column(db.DateTime, default=datetime.now)
    event_date = db.Column(db.DateTime, nullable=True)
    order = db.Column(db.Integer, default=0)  # Per ordinare le foto
    
    tournament = db.relationship('Tournament', back_populates='photos')
    category = db.relationship('PhotoCategory', back_populates='photos')